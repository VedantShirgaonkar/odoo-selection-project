...
const ReWearAuth = () => {
 ...
 const handleInputChange = (field, value) => {
  setFormData(prev => {
    ...
    setValidations(prevValid => {
      let updatedValidations = { ...prevValid };
      if (field === 'email') {
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        updatedValidations.email = emailRegex.test(value);
      }
      if (field === 'password') {
        updatedValidations.password = value.length >= 8;
        updatedValidations.confirmPassword = updated.confirmPassword === value;
      }
      if (field === 'confirmPassword') {
        updatedValidations.confirmPassword = value === updated.password;
      }
      if (field === 'firstName' || field === 'lastName') {
        updatedValidations[field] = value.length >= 2;
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md">
        ...
        {/* Auth Form */}
        <form onSubmit={handleSubmit} className={`bg-white rounded-2xl shadow-xl p-8 transform transition-all duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
